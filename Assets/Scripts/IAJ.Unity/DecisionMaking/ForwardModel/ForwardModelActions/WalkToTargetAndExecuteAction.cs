...
namespace Assets.Scripts.IAJ.Unity.DecisionMaking.ForwardModel.ForwardModelActions
{
    public abstract class WalkToTargetAndExecuteAction : Action
    {
        ...
        {
            var distance = this.Character.AStarPathFinding.Heuristic.H(this.Character.AStarPathFinding.Quantize(currentPosition),
                                                                       this.Character.AStarPathFinding.Quantize(this.Target.transform.position));
        ...
        {
            var position = (Vector3)worldModel.GetProperty(Properties.POSITION);
            var target =  (Vector3)this.Target.transform.position;
            var distance = (position-target).magnitude;
